...
WebDriverWait(web,10,0.2).until(lambda x:x.find_element(By.XPATH,'//*[@id="ConditionKey"]'))
...
WebDriverWait(web,10,0.2).until(lambda x:x.find_element(By.XPATH,'//*[@id="ConditionValue"]'))
...
WebDriverWait(web,10,0.2).until(lambda x:x.find_element(By.XPATH,'//*[@id="docList"]/div[1]/div[2]')) # 如assert bool(el_ls)==True处抛出AssertionError，请增加此处时间
...
def GetData():
    data_ls=[]
    WebDriverWait(web,10,0.2).until(lambda x:x.find_element(By.XPATH,'//*[@id="docList"]/ul/li/dl/dd/div[1]/a'))
...
print(f'存储完毕，程序总用时{round(time.time()-time_start-(interval_end-interval_start),2)}秒。')

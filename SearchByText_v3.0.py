...
#         按梯度设置等待时间，以同时保证运行速度和数据提取完整（按照一般网速）
        if i+2<=10:
            ...
            time.sleep(7)
        print(f'……正在加载第{i+2}/{pages}页……',end='\r')
#         WebDriverWait(web,15,0.3).until(lambda x:x.find_element(By.LINK_TEXT,'下一页'))
...
try:
    ...
    SaveToMd()
    print(f'★存储Markdown完毕，程序总用时{round((time.time()-time_start-(interval_end-interval_start))/60,2)}分钟。即将打开文件：{os.path.abspath(savepath)}')
    os.system(savepath)
except Exception as e:
